<script>
    ...
    function submit(e) {
        e.preventDefault();
        $form.put(`/suppliers/${supplier.uuid}`);
    }
</script>

<svelte:head><title>Editar proveedor</title></svelte:head>
...
<AppLayout>
    <div class="flex-1 overflow-y-auto">
        <div class="border-b border-slate-800 px-8 py-5 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl font-semibold text-white">Editar proveedor</h1>
                <p class="text-sm text-slate-400 mt-0.5">{supplier.name}</p>
            </div>
        </div>

        <div class="p-8 max-w-2xl">
            <Breadcrumb items={[{ label: 'Proveedores', href: '/suppliers' }, { label: supplier.name, href: `/suppliers/${supplier.uuid}` }, { label: 'Editar' }]} />
